...
Cualquier desarrollador, ingeniero, analista de datos o empresa que necesite un almacenamiento a prueba de manipulaciones. Al ser una herramienta multipropósito, no está limitada a una sola industria.

---

## 🚀 ¿Qué se puede hacer con esta herramienta? (Casos de Uso)

Gracias a su flexibilidad clave-valor, puedes integrar este SGBD en múltiples áreas:
...
* **`src/app/page.tsx`**: Es el panel de control gráfico (DApp). Se encarga de capturar la entrada del usuario, solicitar la firma a través de la Phantom Wallet y enviar la transacción RPC a la red de Solana. Incluye un sistema dinámico multilingüe (ES, EN, PT) y rotación de ejemplos.
* **`src/idl.json`**: El *Interface Description Language*. Es el "diccionario traductor" fundamental. Le explica al código de JavaScript/TypeScript qué forma tiene el contrato de Rust, qué funciones existen y qué argumentos necesitan para que la comunicación sea perfecta.
...
Abre `http://localhost:3000` en tu navegador. Necesitarás la extensión **Phantom Wallet** configurada en la red `Devnet` con algo de saldo (SOL de prueba) para firmar las transacciones.
